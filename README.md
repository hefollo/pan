...
- PHP >= 7.1，需启用 cURL 扩展
- MySQL / MariaDB
- 当前程序版本 `VERSION` 为 `1613`，数据库版本 `DB_VERSION` 为 `1018`
...
> 静态资源由 `VERSION` 常量做缓存串（`style.css?v=1613`）。如果自行修改了 CSS/JS，记得同步提高 `includes/common.php` 里的 `VERSION`，否则浏览器会继续使用旧缓存；站点前面挂了 CDN 的还需刷新 CDN。
