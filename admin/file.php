	...
	$("#listTable").bootstrapTable({
		...
		columns: [
			...
			},
			{
				...
				align: 'center',
				formatter: function(value, row, index) {
					var viewurl = escapeHtml(row.viewurl);
					var thumburl = escapeHtml(row.thumburl || row.viewurl);
					var name = escapeHtml(row.name);
					var icon = escapeHtml(row.icon);
					if(row.view && row.view_type == 'image'){
						return '<button type="button" class="admin-file-thumb js-image-preview" data-src="'+viewurl+'" title="点击查看大图"><img src="'+thumburl+'" alt="'+name+'" loading="lazy"></button>';
					}
					return '<span class="admin-file-icon"><i class="fa '+icon+'"></i></span>';
				}
			},
			{
				field: 'name',
				title: '文件名',
				formatter: function(value, row, index) {
					var fileurl = escapeHtml(row.fileurl);
					var viewurl = escapeHtml(row.viewurl);
					var name = escapeHtml(value);
					var preview = '';
					if(row.view){
						if(row.view_type == 'image'){
							preview = '<div class="admin-file-sub"><a href="javascript:void(0)" class="js-image-preview" data-src="'+viewurl+'">查看大图</a></div>';
						}else{
							preview = '<div class="admin-file-sub"><a href="javascript:showfile('+row.id+',\''+row.view_type+'\')">预览</a></div>';
						}
					}
					return '<div class="admin-file-info"><a class="admin-file-name" href="'+fileurl+'" title="点击下载">'+name+'</a>'+preview+'</div>';
				}
			},
			{
				field: 'size',
			...
			},
			{
				...
				formatter: function(value, row, index) {
					return '<div class="admin-action-buttons"><a href="javascript:editframe('+row.id+')" class="btn btn-xs btn-info">编辑</a><a href="'+row.pageurl+'" class="btn btn-xs btn-warning" target="_blank">查看</a><a href="javascript:delFile('+row.id+')" class="btn btn-xs btn-danger">删除</a></div>';
				}
			},
		],
	})
