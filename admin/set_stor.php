		...
		<div class="panel-body">
			<form onsubmit="return saveSetting(this)" method="post" class="form-horizontal" role="form">
				<div class="form-group">
					...
					<div class="col-sm-9"><select class="form-control" name="storage" default="<?php echo $conf['storage']?>"><option value="local">本地存储</option><option value="oss">阿里云OSS</option><option value="qcloud">腾讯云COS</option><option value="obs">华为云OBS</option><option value="upyun">又拍云</option><option value="qiniu">七牛云</option><option value="s3">通用 S3</option><?php if (defined('SAE_ACCESSKEY')) {?><option value="sae">SaeStorage</option><?php }?></select><font color="green">已有文件的情况下请勿随意变更，否则之前上传的文件全部无法下载</font></div>
				</div><br/>
...
<?php }?>
	...
	<div class="panel panel-info">
		<div class="panel-heading"><h3 class="panel-title"><a data-toggle="collapse" data-parent="#accordion" href="#stor_s3" class="collapsed">通用 S3 兼容存储</a></h3></div>
		<div id="stor_s3" class="panel-collapse collapse">
		<div class="panel-body">
			<form onsubmit="return saveSetting(this)" method="post" class="form-horizontal" role="form">
				<div class="form-group">
					<label class="col-sm-3 control-label">Access Key</label>
					<div class="col-sm-9"><input type="text" name="s3_ak" value="<?php echo isset($conf['s3_ak']) ? $conf['s3_ak'] : ''; ?>" class="form-control"/></div>
				</div><br/>
				<div class="form-group">
					<label class="col-sm-3 control-label">Secret Key</label>
					<div class="col-sm-9"><input type="password" name="s3_sk" value="<?php echo isset($conf['s3_sk']) ? $conf['s3_sk'] : ''; ?>" class="form-control" autocomplete="new-password"/></div>
				</div><br/>
				<div class="form-group">
					<label class="col-sm-3 control-label">Endpoint</label>
					<div class="col-sm-9"><input type="text" name="s3_endpoint" value="<?php echo isset($conf['s3_endpoint']) ? $conf['s3_endpoint'] : ''; ?>" class="form-control" placeholder="https://s3.example.com"/><font color="green">请填写完整地址，支持 AWS S3、MinIO、R2、B2 等兼容服务</font></div>
				</div><br/>
				<div class="form-group">
					<label class="col-sm-3 control-label">Region</label>
					<div class="col-sm-9"><input type="text" name="s3_region" value="<?php echo isset($conf['s3_region']) ? $conf['s3_region'] : 'us-east-1'; ?>" class="form-control" placeholder="us-east-1"/></div>
				</div><br/>
				<div class="form-group">
					<label class="col-sm-3 control-label">Bucket</label>
					<div class="col-sm-9"><input type="text" name="s3_bucket" value="<?php echo isset($conf['s3_bucket']) ? $conf['s3_bucket'] : ''; ?>" class="form-control"/></div>
				</div><br/>
				<div class="form-group">
					<label class="col-sm-3 control-label">对象前缀</label>
					<div class="col-sm-9"><input type="text" name="s3_prefix" value="<?php echo isset($conf['s3_prefix']) ? $conf['s3_prefix'] : 'file/'; ?>" class="form-control" placeholder="file/"/><font color="green">留空表示存储到 Bucket 根目录</font></div>
				</div><br/>
				<div class="form-group">
					<label class="col-sm-3 control-label">地址风格</label>
					<div class="col-sm-9"><select class="form-control" name="s3_path_style" default="<?php echo !empty($conf['s3_path_style']) ? '1' : '0'; ?>"><option value="0">虚拟主机风格（bucket.endpoint）</option><option value="1">路径风格（endpoint/bucket）</option></select><font color="green">MinIO 或 Bucket 名包含点时通常选择路径风格</font></div>
				</div><br/>
				<div class="form-group">
					<div class="col-sm-offset-3 col-sm-9"><input type="submit" name="submit" value="修改" class="btn btn-primary btn-block"/></div>
				</div>
			</form>
		</div>
		</div>
